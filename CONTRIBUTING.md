# Contributing to Cobalt 2

Thank you for your interest in contributing to Cobalt 2! Pull requests are welcome.
...
Each plugin has its own file in `lua/cobalt2/groups/` that defines the highlight groups for that plugin. A typical plugin module looks like this:
...
For the complete list of available colors, see [colors.lua](lua/cobalt2/colors.lua).
...
2. **Create the plugin file**: Create a new file `lua/cobalt2/groups/plugin-name.lua`:
...
3. **Register the plugin**: Add an entry to the `M.plugins` table in `lua/cobalt2/groups/init.lua`:
...
- Look at similar highlight groups in [base.lua](lua/cobalt2/groups/base.lua) for consistency
...
vim.cmd("colorscheme cobalt2-storm")
vim.cmd("colorscheme cobalt2-moon")
...
1. Create `lua/cobalt2/groups/cool-finder.lua`:
...
2. Add to `lua/cobalt2/groups/init.lua`:
...
3. **Use template variables**: Reference colors using `${color_name}` syntax within the template string. Common colors include:
   ...
   - `${_style_name}` - the theme variant name (e.g., "Cobalt 2 Storm")
   - `${_name}` - the base name (e.g., "cobalt2_storm")
...
5. **Follow existing patterns**: Look at other extras like [kitty.lua](lua/cobalt2/extra/kitty.lua) or [alacritty.lua](lua/cobalt2/extra/alacritty.lua) for examples.

### How to Add a New Extra

1. Create a file like `lua/cobalt2/extra/cool-app.lua` following the template structure above.

2. Add the name and output file extension to the `extras` table in `lua/cobalt2/extra/init.lua`:
...
3. Run the build script from the cobalt2 plugin directory:
