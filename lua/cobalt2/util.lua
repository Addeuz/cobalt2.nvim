...
function M.invert(color)
  ...
  elseif type(color) == "string" then
    local hsluv = require("cobalt2.hsluv")
    if color ~= "NONE" then
...
---@param groups cobalt2.Highlights
...
function M.cache.file(key)
  return vim.fn.stdpath("cache") .. "/cobalt2-" .. key .. ".json"
end

---@param key string
function M.cache.read(key)
  ---@type boolean, cobalt2.Cache
