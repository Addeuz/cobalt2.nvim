...
local Groups = require("cobalt2.groups")
...
function M.extras()
  local Extra = require("cobalt2.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/cobalt2/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/cobalt2/config.lua", "\n(--@class cobalt2%.Config.-\n})")
